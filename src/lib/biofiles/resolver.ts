...
export class BioCIDResolver {
  ...
  async discoverAllBioFiles(verbose: boolean = false): Promise<BioFile[]> {
    ...
    // Data Source 1: Story Protocol IP Assets (includes BioIP files registered on Story)
    try {
      if (verbose) console.log('🔍 Fetching Story Protocol assets...');
      const ipAssets = await this.api.getStoryIPAssets();
      if (verbose) console.log(`✅ Found ${ipAssets.length} Story Protocol assets`);
      ...
      for (const asset of ipAssets) {
        ...
        bioFiles.push({
          filename,
          biocid: `biocid://${asset.owner || asset.wallet_address || 'unknown'}/story/${asset.ipId}`,
          type: fileType,
          source: 'Story',
      ...
      if (verbose) console.error('❌ Error fetching Story Protocol assets:', error);
    }
