...
export async function svCallExecCommand(opts: SvCallExecOptions): Promise<void> {
  ...
  // reference: prefer the EXACT assembly the modBAM was aligned to (assembly38, with-alt contigs);
  ...
  if (!refRel) { logLine('[sv-call] no reference fasta (+.fai) found'); uploadAudit(); process.exit(1); }
  // optional tandem-repeat bed improves Sniffles2 SV calls in repeat regions
  let trArgs = '';
