...
export async function ontVariantsExecCommand(opts: OntVariantsExecOptions): Promise<void> {
  ...
  // reference: the EXACT assembly the modBAM was aligned to (assembly38, with-alt contigs)
  let refRel = '';
  ...
  if (!refRel) { logLine('[ont-variants] no reference fasta (+.fai) found'); uploadAudit(); process.exit(1); }
