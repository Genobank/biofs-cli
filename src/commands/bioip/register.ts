...
export async function tokenizeCommand(filePath: string, options: TokenizeOptions): Promise<void> {
  ...
  try {
    ...
    formData.append('network', network.name);
    ...
    if (error.response?.data?.status_details?.description?.includes('already tokenized') ||
      ...
      if (network.name === 'Sequentia v4 Mainnet') {
        throw new Error('This genomic profile has already been tokenized on Sequentia (duplicate fingerprint detected)');
      }
